---
id: sccwrp_tr1289
title: "Southern California Bight 2018 Regional Monitoring Program: Volume III. Benthic Infauna"
steward: Southern California Coastal Water Research Project
url: https://ftp.sccwrp.org/pub/download/DOCUMENTS/JournalArticles/1289_Bight18BenthicInfauna.pdf
doi: null
status: VERIFIED
tier: FETCHED
access:
  - Open
    https://www.sccwrp.org/about/research-areas/regional-monitoring/southern-california-bight-regional-monitoring-program/bight-program-documents/,
    SCCWRP's "Bight Program Documents" page
  - Expand the "Bight '18" section, then its "Assessment Reports" sub-section, and follow the link
    labelled "Volume III – Benthic Infauna", which points to
    https://ftp.sccwrp.org/pub/download/DOCUMENTS/JournalArticles/1289_Bight18BenthicInfauna.pdf
  - Download that file over HTTPS; no account, key or referrer is required (HTTP 200, 9,780,729
    bytes, Content-Type application/pdf, Last-Modified "Sat, 18 Jan 2025 00:14:02 GMT", 2026-09-15)
  - The report's number is on its cover as "SCCWRP Technical Report 1289", but the file is served
    from the DOCUMENTS/JournalArticles/ path, not DOCUMENTS/TechnicalReports/. The same file name
    under TechnicalReports/ answers HTTP 404 with 321 bytes (2026-09-15)
format: >-
  PDF; served as Content-Type application/pdf
license: >-
  not stated: the report prints no copyright or licence statement. The page linking it,
  https://www.sccwrp.org/about/research-areas/regional-monitoring/southern-california-bight-regional-monitoring-program/bight-program-documents/,
  states no terms and links no terms, licence or disclaimer page; its footer carries
  "© 2026 — Southern California Coastal Water Research Project. All Rights Reserved". The one
  licence statement inside the PDF, "0025-326X/ © 2020 The Authors. Published by Elsevier Ltd.
  This is an open access article under the CC BY license
  (http://creativecommons.org/licenses/by/4.0/)", is printed on the reprinted journal manuscript
  that Appendix E reproduces, and is that manuscript's licence rather than the report's (all
  retrieved 2026-09-15)
variables:
  - "abundance, species richness, diversity, and evenness"
  - Benthic Response Index (BRI)
  - California Sediment Quality Objectives Benthic Line of Evidence (SQO BLOE)
  - Multivariate AZTI Marine Biotic Index (M-AMBI)
  - >-
    "sediment chemistry (metals, PAHs, PCBs, and DDTs)", which the report compared "at the Channel
    Islands stratum"
  - >-
    "sediment grainsize", which the report compared "at the Channel Islands stratum"
  - >-
    "water quality (bottom water temperature, dissolved oxygen, and salinity)", which the report
    compared "at the Channel Islands stratum" and states "were obtained from CalCOFI bottle samples
    collected near (within 37 km) and at the same depth (+/- 6m) as the benthic samples"
  - >-
    "modelled measures of ocean acidification (pH, aragonite saturation, calcite saturation, and
    pCO2)", which the report compared "at the Channel Islands stratum" and states "were calculated
    using linear regression models applied to CalCOFI water quality data"
  - >-
    Chlorophyll A, in the caption of Figure 18: "Chlorophyll A, dissolved oxygen (DO), salinity,
    and temperature values in the Channel Islands for each Bight Survey (1998, 2003, 2008, 2013,
    and 2018)"
coverage: >-
  "The survey area for the 2018 Southern California Bight Regional Monitoring Program (Bight’18)
  spanned from Point Conception, CA in the north to the US-Mexico border in the south and from the
  mainland coastal embayments west to the Channel Islands (Figure 1). The soft sediment portions of
  this region less than 1,000 m deep were divided into eleven strata based upon known biogeographic
  breaks in community composition (e.g., estuaries or deep continental shelf) or area of different
  regulatory/management interest (e.g., ports or continental slope) (Table 1)." Table 1, which its
  caption says gives "the range of depth at which those stations were located", states "7-30" for
  the "Inner Shelf" stratum, under a column header reading "Depth Range" over the unit "(m)".
  "Sediment samples for benthic macrofauna analysis were collected from July 1 to September 28,
  2018." "Samples of benthic
  macrofauna were successfully collected at 376 sites across the Southern California Bight";
  "Macrobenthic community composition was assessed for the continental shelf portions of the
  Southern California Bight (6-200 m deep) and embayments. These areas represent approximately 36%
  of the total area of the Southern California Bight." The report presents "the trends in that
  condition over the last 20 years (1998-2018)". It also presents "Aragonite saturation, calcite
  saturation, partial pressure of CO2, and pH values in the Channel Islands for each Bight Survey
  (1998, 2003, 2008, 2013, and 2018)" and "Chlorophyll A, dissolved oxygen (DO), salinity, and
  temperature values in the Channel Islands for each Bight Survey (1998, 2003, 2008, 2013, and
  2018)".
coverage_stated_at: >-
  Chapter II, Methods, Study Design, printed page 3 (PDF page 20); then Table 1, printed page 4
  (PDF page 21); then Chapter II, Methods, Sample Processing, printed page 4 (PDF page 21); then
  the Executive Summary, printed page v (PDF page 7); then the captions of Figures 17 and 18,
  printed pages 40 and 41 (PDF pages 57 and 58), all of
  https://ftp.sccwrp.org/pub/download/DOCUMENTS/JournalArticles/1289_Bight18BenthicInfauna.pdf,
  retrieved 2026-09-15. The "at the Channel Islands stratum" scope in `variables` is Chapter II,
  Methods, Data Analysis, printed page 9 (PDF page 26)
retrieved: 2026-09-15
fetch_script: src/fetch/sccwrp_tr1289.py
file: null
transcribed_from: null
topics:
  - waves-storms-sediment/sediment-sand
  - ocean-climate/temperature
  - ocean-climate/oxygen-ph
  - water-quality-harvest/discharges-outfalls
regions:
  - scb
beds: []
sites: []
references: []
human_task: null
---
