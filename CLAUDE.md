...
- A PR is ready when it has been audited, unless the owner says to skip. The `audit-pr` skill
  ...
  found. An agent rules on every finding, at any severity, and records why — where the finding is,
  in the PR body or the issue thread, not only in the session that made the ruling — before it moves
  on. It does not wait for the owner to accept the ruling.
