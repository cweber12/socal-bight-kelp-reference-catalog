...
- **Every table's intro.** The intro, not the row, carries a freeze and its exit trigger
  (`docs/prd/regions-and-authorities.md`, "**Freeze, 2026-09-14**"), the auto-merge declaration
  (`CLAUDE.md`, "Auto-merge in a declared PRD run"), which rows share one issue, and where the work
  order departs from the numbers in the `order` column.
...
Six things a reader gets right and a column reader does not. Each carries the row it was measured
...
- **A `#` cell can say `to file`** — the row is planned and its issue does not exist yet.
  `re-entry.md` has three such rows: S2 and S3 in Part one, P1 in Part three. Such a row is not in
  the order and is reported instead.
...
- **The order is the rows' document order, and the `order` cell is a label.** Sorting by that cell
  ...
  `16b`, `16c`, `16d`, `16e`, `16`, `16a`, `17`, `18`, `18a` down the page, and its intro says why —
  "Between 16b and 16 sit 16c, 16d and 16e, in that order", and "18a follows 16e, not in any order".
  `re-entry.md`'s three tables label their rows `S1`–`S4`, `M1`–`M15`, and `C1`, `C2`, `P1`, `X1`.
...
The `gh` forms are `docs/agents/issue-tracker.md`, "Commands". One listing covers every row of every
table:
...
- **Absent from that listing** — the issue is closed, so the row is done, whatever the row says.
...
**STOP if a row names an issue `gh` cannot open.** The table is ahead of the tracker or behind it,
and which of the two is wrong is not this skill's to decide.
...
The order goes on the **first entry's issue** — the row the run would work next — and #228's per-row
...
- **An open `ready-for-agent` issue on the milestone that no row names.** It is invisible to this
  ...
  milestone 6.2, labelled `ready-for-agent`, and in neither ordering". Where a table's intro names
  its milestone, list any such issue under "not in the order" and point at that entry. Do not order
  it, and do not edit the table to add it.
