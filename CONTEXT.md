...
| `variables`* | list of `{name, description, unit, file?}`, no other key; until #182 an entry may be a str instead, one form to a list | the named columns or fields of the source's data files, as the source states them. `name` is the column or field and `file` the held files an entry is stated for, each spelled as the `site_key` row spells a column name and a file; `name` is never null, a column the source names nowhere being no entry. `description` and `unit` are what the source states for that column, `null` where it states none. Where a column's `description` or `unit` differs from one of the source's files to another, none is chosen over another and each statement is its own entry; a column several files state the same way is one entry whose `file` names each. `file` is required where the record holds more than one data file and absent where it holds one — for a record whose files `fetch_script` stores, how many it holds is not in the repo until `data-lock.json` (*Gates*), so review checks that and no gate does. The CSV's columns when DERIVED; empty list if NOT HELD |
