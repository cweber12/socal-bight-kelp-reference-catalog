...
would have to be relative to the notebook the cell sits in, and `provenance` does not
know which notebook that is and cannot be told without the caller passing its own path.
The id is what the record is filed under, so a reader has the name they need to find it.
...
positions because nothing in the catalog states an order for a source's files, so an
index would be the directory listing's order dressed up as a fact - and because two of
the three sources held today are multi-file (`calcofi` holds a cast file and a bottle
file, which are different data and cannot be picked apart by position).

`load()` resolves through the manifests rather than by listing the directory. A manifest
is the record that a fetch wrote that file, so a payload no manifest names - a file left
behind by an earlier fetch, or dropped in by hand - is not handed to a figure as current.
...
class Provenance:
    ...
    first execution is #48 - so what a committed cell's output looks like is not
    something these tests can show.
    """
...
def load(source_id: str, root: Path | None = None) -> Mapping[str, Path]:
    ...
    manifests = sorted(here.glob(f"{MANIFEST_PREFIX}*{MANIFEST_SUFFIX}")) if here.is_dir() else []
    if not manifests:
        raise FileNotFoundError(
