"""Fetch the SBC LTER annual kelp forest biomass table into data/raw/sbc_lter_kelp_biomass/.

Run:  python src/fetch/sbc_lter_kelp_biomass.py
...
This is the shape of src/fetch/sbc_lter_bottom_temperature.py. FILES holds PASTA
identifiers, not URLs, each with the SHA-1 and the size DataONE's system metadata stated
for it on 2026-09-19; the identifier carries the package revision, 18. EDI's own PASTA
service answers HTTP 403 to an anonymous request for them, and the DataONE member node
EDI replicates to serves the same object under the same identifier
(catalog/sources/sbc_lter_kelp_biomass.md, `access`; CONTEXT.md, "Vocabularies", the
...
SOURCE_ID = "sbc_lter_kelp_biomass"
USER_AGENT = "kelpcatalog/knb-lter-sbc.50"
...
FILES = (
    (
        ...
        "a2a2ee412caa5a41af9d6d80f29b8a7fe09acd72",
        48203257,
    ),
)
CHUNK = 1 << 20
